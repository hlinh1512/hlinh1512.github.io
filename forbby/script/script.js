...
function Cactus(gameWidth, groundY){
    this.width = 50;    //fixed width cactus
    this.height = (Math.random() > 0.5) ? 40 : 60// two different cactus
    this.x = gameWidth;
...
Cactus.prototype.draw = function(context){
    var oldFill = context.fillStyle;
    cactus_img = new Image();
    cactus_img.src = 'img/hlinh.png';
    context.drawImage(cactus_img, this.x, this.y, this.width, this.height);
};
    ...
    for(i = 0; i < this.cacti.length; i++){
        if(
            ...
            && leftWall(this.dino) <= rightWall(this.cacti[i]) && bottomWall(this.dino) >= topWall(this.cacti[i]))
            {
                // COLLISION OCCURED
                this.paused = true;
                alert("Em thua roi do ngok ai lop my bby bububu")
            }
            this.noOfFrames++;
